...
const securityMiddleware = async (req, res, _next) => {
  try {
    ...
    if(decision.isDenied() && decision.reason.isShield()){
      ...
      return res.status(403).json({error:'Forbidden', message: 'Automated Requests Blocked by security policy!'});
    }
    ...
    res.status(500).json({error: 'Internal server Error :', message:'Something went wrong security middleware'});
  }
};
